...
func GetCategoryName(CategoryID string) string {
	...
	conf.DB.Model(Category{}).Select("category_name").Where("category_id = ?", CategoryID).Scan(&categoryName)
	return categoryName.CategoryName
}

func GetCategoryList(limit, page int) ResCategoryList {
	var resCategoryList ResCategoryList
	conf.DB.Model(Category{}).Order("created_at desc").Limit(limit).Offset((page - 1) * limit).Find(&resCategoryList.CategoryList)
	conf.DB.Model(Category{}).Count(&resCategoryList.Count)
...
func UpdateCakeCategory(categoryName, categoryId string) bool {
	...
	if conf.DB.Model(Category{}).Where("category_id = ?", categoryId).Updates(data).RowsAffected > 0 {
		return true
	} else {
		return false
	}
...
func DeleteCakeCategory(categoryId string) bool {
	if conf.DB.Where("category_id = ?", categoryId).Delete(Category{}).RowsAffected > 0 {
		return true
	} else {
		return false
	}
